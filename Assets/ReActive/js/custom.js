...
(function (a) {
    ...
    var e = function (b) {
        ...
        if (!d.is("label")) {
            ...
            if (d.is("label")) {
                var e = b.attr("id");
                if (e) {
                    d = c.find('label[for="' + e + '"]')
                }
            }
        }
        if (d.is("label")) {
            return d.css("cursor", "pointer")
        }
        return false
    };
    var f = function (b) {
    ...
    var i = function (b) {
        var c;
        a(".jqTransformSelectWrapper select", b)
            .each(function () {
        ...
        a("a.jqTransformCheckbox, a.jqTransformRadio", b)
            .removeClass("jqTransformChecked");
        a("input:checkbox, input:radio", b)
            .each(function () {
            if (this.checked) {
                a("a", a(this)
                    .parent())
    ...
    a.fn.jqTransCheckBox = function () {
        return this.each(function () {
            ...
            var f = a('<a href="#" class="jqTransformCheckbox"></a>');
            b.addClass("jqTransformHidden")
                .wrap('<span class="jqTransformCheckboxWrapper"></span>')
                .parent()
                .prepend(f);
            b.change(function () {
                this.checked && f.addClass("jqTransformChecked") || f.removeClass("jqTransformChecked");
                return true
            });
            f.click(function () {
                if (b.attr("disabled")) {
    ...
    a.fn.jqTransRadio = function () {
        return this.each(function () {
            ...
            var d = a('<a href="#" class="jqTransformRadio" rel="' + this.name + '"></a>');
            b.addClass("jqTransformHidden")
                .wrap('<span class="jqTransformRadioWrapper"></span>')
                .parent()
                .prepend(d);
            b.change(function () {
                c.checked && d.addClass("jqTransformChecked") || d.removeClass("jqTransformChecked");
                return true
            });
            d.click(function () {
                if (b.attr("disabled")) {
                ...
                b.trigger("click")
                    .trigger("change");
                a('input[name="' + b.attr("name") + '"]', c.form)
                    .not(b)
                    .each(function () {
                    a(this)
                        .attr("type") == "radio" && a(this)
                        .trigger("change")
                });
                return false
            });
            c.checked && d.addClass("jqTransformChecked")
        })
    };
    a.fn.jqTransSelect = function () {
        return this.each(function (b) {
            ...
            var d = e(c);
            var g = c.addClass("jqTransformHidden")
                .wrap('<div class="jqTransformSelectWrapper"></div>')
                .parent()
                .css({
                zIndex: 10 - b
            });
            g.prepend('<div><span></span><a href="#" class="jqTransformSelectOpen"></a></div><ul></ul>');
            var h = a("ul", g).css("width", c.width()).hide();
            a("option", this).each(function (b) {
                var c = a('<li><a href="#" index="' + b + '">' + a(this).html() + "</a></li>");
                h.append(c)
            });
            h.find("a").click(function () {
                a("a.selected", g).removeClass("selected");
                a(this).addClass("selected");
				
                if (c[0].selectedIndex != a(this).attr("index") && c[0].onchange) {
                    c[0].selectedIndex = a(this).attr("index");
                    c[0].onchange()
                }
                c[0].selectedIndex = a(this).attr("index");
				
                a("span:eq(0)", g).html(a(this).html());
				jQuery(c[0]).find('option').removeAttr('selected');
				jQuery(c[0]).find('option').eq(a(this).attr("index")).attr('selected', 'selected').trigger('change');
                h.hide();
				
                return false
            });
            a("a:eq("+this.selectedIndex+")", h).click();
            a("span:first", g)
                .click(function () {
                a("a.jqTransformSelectOpen", g)
                    .trigger("click")
            });
            d && d.click(function () {
                a("a.jqTransformSelectOpen", g)
            ...
            this.oLabel = d;
            var i = a("a.jqTransformSelectOpen", g)
                .click(function () {
            ...
            var j = c.outerWidth();
            var k = a("span:first", g);
            var l = j > k.innerWidth() ? j + i.outerWidth() : g.width();
            g.css("width", l);
            h.css("width", l - 2);
            k.css({
                width: j
    ...
    a.fn.jqTransform = function (c) {
        ...
        return this.each(function () {
            ...
            a("input:checkbox:not(.ios)", this)
                .jqTransCheckBox();
            a("input:radio", this)
                .jqTransRadio();
            if (a("select:not(.chzn-select)", this)
                .jqTransSelect()
                .length > 0) {
                h()
            }
...
jQuery(document).ready(function() {
	...
	jQuery('.nav li.has-menu > a').live('click', function(event) {
		...
		var plus = t.find('span').hasClass('plus');
		
		if(plus) {
			t.find('span.plus').removeClass('plus').addClass('minus');	
		}else {
			t.find('span.minus').removeClass('minus').addClass('plus');	
		}
		
		if(jQuery('.nav li').hasClass('expanded')) {
			jQuery('.nav').find('li.expanded ul.submenu').slideToggle();	
			jQuery('.nav').find('li.expanded span.minus').removeClass('minus').addClass('plus');
			jQuery('.nav li').removeClass('expanded');
	...
	function resize_nav() {
		...
		var nav_height = $('nav#sidebar').height();
		if(nav_height <= window_height) {
			$('nav#sidebar').css({'position':'fixed'});	
			$('header figure#logo').css({'position':'fixed'});
		}else {
			$('nav#sidebar').css({'position':'absolute'});	
			$('header figure#logo').css({'position':'absolute'});
		}
	}
	...
	if (jQuery('table.data-table').length) {
		jQuery('table.data-table').dataTable({ "sPaginationType": "full_numbers" });
	}
	...
	if (jQuery('#inputDate').length) {
		jQuery('#inputDate').DatePicker({
			...
			onBeforeShow: function(){
				jQuery('#inputDate').DatePickerSetDate($('#inputDate').val(), true);
			},
	...
	jQuery('#nav_select').live('change', function() {
		var t = jQuery(this);
