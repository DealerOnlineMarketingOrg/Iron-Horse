<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class DOM_Controller extends CI_Controller {
		...
		public function __construct() {
			...
			$access_string = $this->uri->uri_string();
			$page_access_level = $this->mods->getAccessLevelByHref($access_string);
			print_object($this->user['DropdownDefault']);
			if($access_string) {
				if($this->user['DropdownDefault']->PermLevel <= $page_access_level->Level) {
					redirect('/','refresh');	
				}
			}
