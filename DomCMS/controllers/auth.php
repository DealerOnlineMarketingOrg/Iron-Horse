...
class Auth extends CI_Controller {
    ...
    public function Login_user() { 
		...
		$check_login_attempts = $this->login_attempts->get_attempts_count($ip_address,$this->security->xss_clean($this->input->post('email')));
		...
		if(!$check_login_attempts >= 6) {
			...
			if($this->form_validation->run() == FALSE) {
				...
				$this->login_attempts->increase_attempt($ip_address,$this->security->xss_clean($this->input->post('email')));
			}else {	
				...
				if(!is_object($user)) {
				    ...
				    $this->login_attempts->clear_attempts($ip_address,$this->security->xss_clean($this->input->post('email')));
